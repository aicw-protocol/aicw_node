...
$WailsJson = Join-Path $GuiDir "wails.json"
$Goos = "windows"
$Goarch = "amd64"

$ProductVersion = (Get-Content $WailsJson -Raw | ConvertFrom-Json).info.productVersion
$VersionTag = "v$ProductVersion"

$InstallerDist = Join-Path $DistDir "aicw-node-setup-$VersionTag-windows-amd64-installer.exe"
...
Write-Host "Product version $VersionTag"
...
$env:GOOS = $Goos
$env:GOARCH = $Goarch
$ldflags = "-X main.guiVersion=$ProductVersion -s -w"
if (Get-Command wails -ErrorAction SilentlyContinue) {
  wails build -platform windows/amd64 -clean -skipbindings -nsis -ldflags $ldflags
} else {
  go run github.com/wailsapp/wails/v2/cmd/wails@v2.10.1 build -platform windows/amd64 -clean -skipbindings -nsis -ldflags $ldflags
}
Pop-Location
